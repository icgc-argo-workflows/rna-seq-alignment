...
process jsonParser {
  ...
  script:
    """
    set -euxo pipefail
    VARIABLE1=`cat ${metadata_analysis} | jq -r 'if ([.read_groups[]?] | length) >0 then [.read_groups[] | .is_paired_end] | all | tostring else "NO_VALUE" end' | tr -d '\\n'`
    PAIRED=\${VARIABLE1:-'NO_VALUE'}
    VARIABLE2=`cat ${metadata_analysis} | jq -r '[.files[] | .info? | .analysis_tools[]?] | unique | join(",")' | tr -d '\\n'`
    ANALYSIS_TOOLS=\${VARIABLE2:-'NO_VALUE'}
    VARIABLE3=`cat ${metadata_analysis} | jq -r 'if (.experiment | .library_strandedness?) then .experiment|.library_strandedness else "NO_VALUE" end' | tr -d '\\n'`
    STRAND=\${VARIABLE3:-'NO_VALUE'}
